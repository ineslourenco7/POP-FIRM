...
import { Activity, Ban, BarChart3, CreditCard, Gift, History, Lock, MessageSquare, Search, Shield, Target, UserCheck, Users, Wallet, X } from "lucide-react";
...
export default function AdminDashboard() {
  const [promotionItems, setPromotionItems] = useState(initialPromotions);
  ...
  const [promoForm, setPromoForm] = useState({ code: "", discount: "", usage: "0/100", expires: "2026-06-30", status: "Active" });
  ...
  const openCreatePromotion = () => {
    setEditingCode(null);
    setPromoForm({ code: "", discount: "", usage: "0/100", expires: "2026-06-30", status: "Active" });
    setShowPromotionForm(true);
    setAdminNotice("A criar nova promoção. Preenche os campos e guarda.");
  };

  const openEditPromotion = (promo: (typeof initialPromotions)[number]) => {
  ...
  const savePromotion = () => {
    ...
    if (!code || !promoForm.discount.trim()) {
      setAdminNotice("Preenche pelo menos o código e o desconto da promoção.");
      return;
    }

    const nextPromo = { ...promoForm, code, discount: promoForm.discount.trim(), usage: promoForm.usage.trim() || "0/100", expires: promoForm.expires.trim() || "2026-06-30" };
    setPromotionItems((current) => editingCode ? current.map((promo) => promo.code === editingCode ? nextPromo : promo) : [nextPromo, ...current]);
    setShowPromotionForm(false);
    setEditingCode(null);
    setAdminNotice(editingCode ? `Promoção ${code} atualizada.` : `Promoção ${code} criada com sucesso.`);
  };
    ...
    <div className="min-h-screen bg-background text-foreground">
      ...
      <main className="mx-auto max-w-[1600px] space-y-6 px-4 py-6 sm:px-6 sm:py-8">
        ...
        <section className="rounded-3xl border border-border bg-card p-5 shadow-xl">
          ...
          {showPromotionForm && (
            <div className="mb-5 rounded-3xl border border-primary/20 bg-background/70 p-5">
              <div className="mb-4 flex items-center justify-between gap-3">
                <div><p className="text-lg font-black">{editingCode ? "Editar promoção" : "Nova promoção"}</p><p className="text-xs text-muted-foreground">Guarda para aparecer imediatamente na lista.</p></div>
          ...
          <div className="grid gap-4 md:grid-cols-3">{promotionItems.map((promo) => <div key={promo.code} className="rounded-3xl border border-border bg-background/60 p-5"><div className="flex items-start justify-between"><div><p className="text-2xl font-black">{promo.code}</p><p className="text-sm text-muted-foreground">{promo.discount}</p></div><Badge value={promo.status} /></div><div className="mt-4 grid grid-cols-2 gap-3 text-sm"><div><p className="text-xs text-muted-foreground">Uso</p><p className="font-black">{promo.usage}</p></div><div><p className="text-xs text-muted-foreground">Expira</p><p className="font-black">{promo.expires}</p></div></div><div className="mt-4 flex flex-wrap gap-2"><button onClick={() => openEditPromotion(promo)} className={secondaryButtonClass}>Editar</button><button onClick={() => deactivatePromotion(promo.code)} className={secondaryButtonClass}>{promo.status === "Inactive" ? "Ativar" : "Desativar"}</button></div></div>)}</div>
        </section>
      </main>
    </div>
  );
