...
function ChallengeCard({ plan }: { plan: Challenge }) {
  ...
  return (
    <div className={`relative flex flex-col rounded-2xl border bg-card overflow-hidden transition-all duration-200 hover:-translate-y-1 hover:shadow-2xl ${instant ? "border-primary/60 ring-1 ring-primary/20" : popular ? "border-blue-500/40 ring-1 ring-blue-500/10" : premium ? "border-purple-500/35 ring-1 ring-purple-500/10" : "border-emerald-500/30 ring-1 ring-emerald-500/10"}`}>
      ...
      {instant && (
        <div className="absolute top-3 right-3 rounded-full bg-primary px-2.5 py-1 text-[10px] font-black uppercase tracking-wider text-primary-foreground shadow-sm z-10">
          Instant Funding
        </div>
      )}

      <div className="relative px-5 pt-5 pb-3">
        <div className={`text-[10px] font-black uppercase tracking-[0.2em] mb-1 ${instant ? "text-primary" : popular ? "text-blue-400" : premium ? "text-purple-400" : "text-emerald-400"}`}>
          {instant ? "Instant" : popular ? "Pro" : premium ? "Elite" : "Explorer"}
        </div>
        ...
        <p className="text-xs text-muted-foreground leading-snug">{instant ? "Conta financiada instantaneamente." : popular ? "O plano mais escolhido." : premium ? "Para traders experientes." : "Ideal para começar."}</p>
      </div>
      ...
      <div className="px-5 py-3 flex-1 space-y-2 text-xs">
        ...
        <div className="flex justify-between border-b border-white/5 pb-2"><span className="text-muted-foreground">Perda Diária Máx.</span><strong>{plan.maxDailyDrawdown}%</strong></div>
