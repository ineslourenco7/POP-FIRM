...
const appVersion = "prop-firm-stable-auth-2026-05-27";
...
function RequireLogin({ children }: { children: ReactNode }) {
  ...
  if (!isLoaded) {
    return <div className="flex min-h-screen items-center justify-center bg-background text-muted-foreground">A carregar sessão...</div>;
  }
...
function RequireAdmin({ children }: { children: ReactNode }) {
  ...
  if (!isLoaded) {
    return <div className="flex min-h-screen items-center justify-center bg-background text-muted-foreground">A validar admin...</div>;
  }
    ...
    <div className="min-h-screen bg-background text-foreground">
      <header className="border-b border-border bg-card/70 px-4 py-3 backdrop-blur">
        <div className="mx-auto flex max-w-6xl items-center justify-between gap-4">
          <div>
            ...
            <h1 className="text-xl font-black md:text-2xl">Área do Trader</h1>
          </div>
          <div className="flex items-center gap-4">
            ...
            <Link href="/challenges" className="text-sm text-muted-foreground hover:text-foreground">Desafios</Link>
      ...
      <main className="mx-auto max-w-6xl px-6 py-10">
        <div className="rounded-3xl border border-border bg-card p-8 shadow-2xl">
          <p className="text-xs font-black uppercase tracking-[0.2em] text-primary">Sessão ativa</p>
          <h2 className="mt-2 text-4xl font-black">Conta criada e autenticada</h2>
          <p className="mt-4 max-w-2xl text-muted-foreground">O acesso ao terminal está funcional. Escolhe um desafio para continuar para o checkout.</p>
      ...
      <main className="relative mx-auto max-w-6xl px-6 py-8 md:py-12">
        ...
        <div className="mt-8 grid gap-6 lg:grid-cols-[1.15fr_0.85fr]">
          <section className="rounded-[2rem] border border-white/10 bg-card/80 p-6 shadow-2xl backdrop-blur md:p-9">
            <div className="mb-6 inline-flex items-center gap-2 rounded-full border border-primary/20 bg-primary/10 px-3 py-1 text-xs font-black uppercase tracking-[0.2em] text-primary">Checkout POP FIRM</div>
