...
const appVersion = "prop-terminal-cfd-polished-pnl-2026-05-26";
  ...
  return <div className="min-h-screen bg-background text-foreground"><header className="border-b border-border bg-card/70 px-4 py-3 backdrop-blur"><div className="mx-auto flex max-w-[1900px] items-center justify-between gap-4"><div><p className="text-xs font-black uppercase tracking-[0.25em] text-primary">POP FIRM Terminal</p><h1 className="text-xl font-black md:text-2xl">Trading Panel</h1></div><div className="flex items-center gap-4"><div className="hidden text-right md:block"><p className="text-xs text-muted-foreground">Conta</p><p className="text-sm font-bold">{user?.primaryEmailAddress?.emailAddress}</p></div><Link href="/admin" className="text-sm text-muted-foreground hover:text-foreground">Admin</Link><UserButton afterSignOutUrl="/" /></div></div></header>
    <main className="mx-auto max-w-[1900px] space-y-3 px-3 py-3">
      ...
      <section className="grid gap-3 xl:grid-cols-[minmax(840px,1fr)_370px]">
        <section className="rounded-3xl border border-border bg-card p-3 shadow-2xl"><div className="mb-3 flex flex-col gap-3 md:flex-row md:items-center md:justify-between"><div><p className="text-xs uppercase tracking-[0.2em] text-muted-foreground">Gráfico principal</p><div className="flex flex-wrap items-baseline gap-3"><h2 className="text-3xl font-black">{selectedAsset.label}</h2><span className="text-sm text-muted-foreground">{selectedAsset.name}</span><span className="rounded-full bg-emerald-500/10 px-2 py-1 text-xs font-bold text-emerald-400">TradingView Online</span></div></div><div className="text-right"><p className="text-xs text-muted-foreground">Preço atual</p><p className="text-2xl font-black tabular-nums text-emerald-300">{formatMarketPrice(selectedPrice, selectedAsset.decimals)}</p></div></div>
          <div className="relative h-[calc(100vh-230px)] min-h-[650px] overflow-hidden rounded-2xl border border-border bg-background"><TradingViewChart symbol={selectedAsset.value} />
            <div className="pointer-events-none absolute left-5 top-5 w-[320px] rounded-2xl border border-border/70 bg-card/85 p-4 shadow-2xl backdrop-blur-xl"><div className="flex items-center justify-between"><span className="text-[10px] font-black uppercase tracking-[0.22em] text-muted-foreground">Open PnL</span><span className="rounded-full bg-background/70 px-2 py-1 text-[10px] font-bold text-muted-foreground">{positions.length} open</span></div><p className={`mt-2 text-4xl font-black leading-none ${openPnl >= 0 ? "text-emerald-300" : "text-red-300"}`}>{formatMoney(openPnl)}</p><div className="mt-3 h-1.5 overflow-hidden rounded-full bg-background"><div className={openPnl >= 0 ? "h-full rounded-full bg-emerald-400" : "h-full rounded-full bg-red-400"} style={{ width: `${Math.min(100, Math.max(8, Math.abs(openPnl) / 30))}%` }} /></div></div>
            <div className="pointer-events-auto absolute right-5 top-5 max-h-[70%] w-[390px] space-y-3 overflow-y-auto pr-1">{activeAssetPositions.map((position) => { const pnl = calculatePnl(position, tick); const current = getFloatingPrice(position.asset, tick); const points = (current - position.entry) * (position.side === "BUY" ? 1 : -1); return <div key={position.id} className={`rounded-3xl border p-4 shadow-2xl backdrop-blur ${pnl >= 0 ? "border-emerald-400/40 bg-emerald-500/20" : "border-red-400/40 bg-red-500/20"}`}><div className="flex items-start justify-between gap-3"><span className={position.side === "BUY" ? "rounded-full bg-emerald-500 px-3 py-1 text-xs font-black text-white" : "rounded-full bg-red-500 px-3 py-1 text-xs font-black text-white"}>{position.side}</span><div className="text-right"><strong className={pnl >= 0 ? "block text-2xl text-emerald-200" : "block text-2xl text-red-200"}>{formatMoney(pnl)}</strong><span className="text-[10px] uppercase tracking-[0.18em] text-muted-foreground">{position.asset.label}</span></div><button onClick={() => closePosition(position.id)} className="rounded-full border border-white/10 bg-black/35 p-1.5 text-white/80 shadow-lg transition hover:bg-red-500 hover:text-white" aria-label="Fechar posição"><X className="h-4 w-4" /></button></div><div className="mt-3 grid grid-cols-2 gap-2 text-xs"><div><p className="text-muted-foreground">Entrada</p><p className="font-black">{formatMarketPrice(position.entry, position.asset.decimals)}</p></div><div><p className="text-muted-foreground">Atual</p><p className="font-black">{formatMarketPrice(current, position.asset.decimals)}</p></div><div><p className="text-muted-foreground">SL</p><p className="font-black">{position.stopLoss ? formatMarketPrice(position.stopLoss, position.asset.decimals) : "—"}</p></div><div><p className="text-muted-foreground">TP</p><p className="font-black">{position.takeProfit ? formatMarketPrice(position.takeProfit, position.asset.decimals) : "—"}</p></div></div><div className={`mt-3 h-1.5 rounded-full ${pnl >= 0 ? "bg-emerald-300" : "bg-red-300"}`} style={{ width: `${Math.min(100, Math.max(12, Math.abs(points / position.entry) * 180000))}%` }} /></div>; })}</div>
          </div></section>
        <aside className="space-y-3"><div className="rounded-3xl border border-border bg-card p-5 shadow-xl"><p className="text-xs font-black uppercase tracking-[0.2em] text-primary">Executar ordem</p><h2 className="mt-1 text-2xl font-black">{selectedAsset.label}</h2><p className="mt-1 text-sm text-muted-foreground">Preço: <strong className="text-foreground tabular-nums">{formatMarketPrice(selectedPrice, selectedAsset.decimals)}</strong></p><div className="mt-5 grid grid-cols-2 gap-3"><div><label className="text-xs text-muted-foreground">Lot size</label><input value={lotSize} onChange={(event) => setLotSize(event.target.value)} className="mt-1 w-full rounded-xl border border-border bg-background px-3 py-3 text-sm font-bold outline-none" /></div><div><label className="text-xs text-muted-foreground">Alavancagem</label><select value={leverage} onChange={(event) => setLeverage(event.target.value)} className="mt-1 w-full rounded-xl border border-border bg-background px-3 py-3 text-sm font-bold outline-none"><option value="10">1:10</option><option value="25">1:25</option><option value="50">1:50</option><option value="100">1:100</option></select></div><div><label className="text-xs text-muted-foreground">Stop Loss</label><input value={stopLoss} onChange={(event) => setStopLoss(event.target.value)} placeholder={formatMarketPrice(selectedPrice * 0.998, selectedAsset.decimals)} className="mt-1 w-full rounded-xl border border-border bg-background px-3 py-3 text-sm font-bold outline-none" /></div><div><label className="text-xs text-muted-foreground">Take Profit</label><input value={takeProfit} onChange={(event) => setTakeProfit(event.target.value)} placeholder={formatMarketPrice(selectedPrice * 1.002, selectedAsset.decimals)} className="mt-1 w-full rounded-xl border border-border bg-background px-3 py-3 text-sm font-bold outline-none" /></div></div><div className="mt-4 grid grid-cols-2 gap-3 rounded-2xl border border-border bg-background/60 p-3 text-center text-sm"><div><p className="text-xs text-muted-foreground">Margem estimada</p><p className="font-black">{formatMoney((selectedPrice * lots * contractMultiplier(selectedAsset)) / leverageValue)}</p></div><div><p className="text-xs text-muted-foreground">Free margin</p><p className="font-black">{formatMoney(freeMargin)}</p></div></div><div className="mt-4 grid grid-cols-2 gap-3"><button onClick={() => openPosition("BUY")} className="rounded-2xl bg-emerald-500 px-4 py-5 text-lg font-black text-white shadow-lg hover:bg-emerald-400">BUY</button><button onClick={() => openPosition("SELL")} className="rounded-2xl bg-red-500 px-4 py-5 text-lg font-black text-white shadow-lg hover:bg-red-400">SELL</button></div><p className="mt-4 rounded-2xl border border-border bg-background/60 p-3 text-xs text-muted-foreground">{lastAction}</p></div>
      ...
      <section className="rounded-3xl border border-border bg-card p-4 shadow-xl"><div className="mb-3 flex items-center justify-between"><h3 className="text-lg font-black">Posições abertas</h3><span className="text-xs text-muted-foreground">SL/TP ativo · fecho manual · sem expiração</span></div><div className="overflow-x-auto"><table className="w-full min-w-[980px] text-sm"><thead className="text-left text-xs uppercase tracking-[0.18em] text-muted-foreground"><tr><th className="py-3">Ativo</th><th>Side</th><th>Entrada</th><th>Atual</th><th>SL</th><th>TP</th><th>Lotes</th><th>Margem</th><th>PnL</th><th className="text-right">Ação</th></tr></thead><tbody>{positions.length === 0 ? <tr><td colSpan={10} className="py-8 text-center text-muted-foreground">Ainda não há posições abertas.</td></tr> : positions.map((position) => { const current = getFloatingPrice(position.asset, tick); const pnl = calculatePnl(position, tick); return <tr key={position.id} className="border-t border-border"><td className="py-4 font-black">{position.asset.label}</td><td className={position.side === "BUY" ? "font-black text-emerald-400" : "font-black text-red-400"}>{position.side}</td><td className="tabular-nums">{formatMarketPrice(position.entry, position.asset.decimals)}</td><td className="tabular-nums">{formatMarketPrice(current, position.asset.decimals)}</td><td>{position.stopLoss ? formatMarketPrice(position.stopLoss, position.asset.decimals) : "—"}</td><td>{position.takeProfit ? formatMarketPrice(position.takeProfit, position.asset.decimals) : "—"}</td><td>{position.lots.toFixed(2)}</td><td>{formatMoney(calculateMargin(position))}</td><td className={pnl >= 0 ? "text-xl font-black text-emerald-400" : "text-xl font-black text-red-400"}>{formatMoney(pnl)}</td><td className="text-right"><button onClick={() => closePosition(position.id)} className="rounded-xl border border-border px-3 py-2 text-xs font-black hover:bg-background">CLOSE</button></td></tr>; })}</tbody></table></div></section>
    </main></div>;
}
