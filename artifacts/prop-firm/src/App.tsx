...
const appVersion = "prop-terminal-cfd-visual-tracking-2026-05-26";
      ...
      <section className="grid gap-3 xl:grid-cols-[minmax(840px,1fr)_370px]">
        <section className="rounded-3xl border border-border bg-card p-3 shadow-2xl"><div className="mb-3 flex flex-col gap-3 md:flex-row md:items-center md:justify-between"><div><p className="text-xs uppercase tracking-[0.2em] text-muted-foreground">Gráfico principal</p><div className="flex flex-wrap items-baseline gap-3"><h2 className="text-3xl font-black">{selectedAsset.label}</h2><span className="text-sm text-muted-foreground">{selectedAsset.name}</span><span className="rounded-full bg-emerald-500/10 px-2 py-1 text-xs font-bold text-emerald-400">TradingView Online</span></div></div><div className="text-right"><p className="text-xs text-muted-foreground">Preço atual</p><p className="text-2xl font-black tabular-nums text-emerald-300">{formatMarketPrice(selectedPrice, selectedAsset.decimals)}</p></div></div>
          <div className="relative h-[calc(100vh-230px)] min-h-[650px] overflow-hidden rounded-2xl border border-border bg-background"><TradingViewChart symbol={selectedAsset.value} />
            <div className="pointer-events-none absolute left-5 top-5 w-[360px] rounded-3xl border border-white/10 bg-black/70 p-4 shadow-2xl backdrop-blur"><p className="text-xs uppercase tracking-[0.2em] text-muted-foreground">Acompanhamento visual</p><p className={`mt-2 text-4xl font-black ${openPnl >= 0 ? "text-emerald-300" : "text-red-300"}`}>{formatMoney(openPnl)}</p><p className="mt-1 text-xs text-muted-foreground">PnL flutuante total das posições abertas</p></div>
            <div className="pointer-events-none absolute right-5 top-5 max-h-[70%] w-[390px] space-y-3 overflow-hidden">{activeAssetPositions.map((position) => { const pnl = calculatePnl(position, tick); const current = getFloatingPrice(position.asset, tick); const points = (current - position.entry) * (position.side === "BUY" ? 1 : -1); return <div key={position.id} className={`rounded-3xl border p-4 shadow-2xl backdrop-blur ${pnl >= 0 ? "border-emerald-400/40 bg-emerald-500/20" : "border-red-400/40 bg-red-500/20"}`}><div className="flex items-center justify-between"><span className={position.side === "BUY" ? "rounded-full bg-emerald-500 px-3 py-1 text-xs font-black text-white" : "rounded-full bg-red-500 px-3 py-1 text-xs font-black text-white"}>{position.side}</span><strong className={pnl >= 0 ? "text-2xl text-emerald-200" : "text-2xl text-red-200"}>{formatMoney(pnl)}</strong></div><div className="mt-3 grid grid-cols-2 gap-2 text-xs"><div><p className="text-muted-foreground">Entrada</p><p className="font-black">{formatMarketPrice(position.entry, position.asset.decimals)}</p></div><div><p className="text-muted-foreground">Atual</p><p className="font-black">{formatMarketPrice(current, position.asset.decimals)}</p></div><div><p className="text-muted-foreground">SL</p><p className="font-black">{position.stopLoss ? formatMarketPrice(position.stopLoss, position.asset.decimals) : "—"}</p></div><div><p className="text-muted-foreground">TP</p><p className="font-black">{position.takeProfit ? formatMarketPrice(position.takeProfit, position.asset.decimals) : "—"}</p></div></div><div className={`mt-3 h-1.5 rounded-full ${pnl >= 0 ? "bg-emerald-300" : "bg-red-300"}`} style={{ width: `${Math.min(100, Math.max(12, Math.abs(points / position.entry) * 180000))}%` }} /></div>; })}</div>
          </div></section>
